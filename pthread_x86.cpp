...
void* LU_threadFunc_NUM_THREADS(void* param) {
    ...
    pthread_exit(NULL);
    return NULL; // Explicit return to silence the warning
}
...
/*
    cal(LU);
    cout << std::fixed << std::setprecision(2); // 设置输出格式，保留两位小数
    cout << "LU：" << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(avx_optimized);
    cout << "avx-LU： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(LU_pthread_dynamic);
    cout << "LU-pthread：" << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(avx_dynamic);
    cout << "avx-LU-pthread：" << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(LU_pthread_dynamic_NUM_THREADS);
    cout << "pthread-s1： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(avx_dynamic_NUM_THREADS);
    cout << "avx-pthread-s1： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(LU_sem_static);
    cout << "pthread-s2： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(sem_static);
    cout << "avx-pthread-s2： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(LU_sem_tri);
    cout << "pthread-s3： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(sem_triplecircle);
    cout << "avx-pthread-s3： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();
*/
    cal(LU_barrier);
    cout << "pthread-s4： " << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();

    cal(barrier_static);
    cout << "avx-pthread-s4：" << (tail - head) * 1000.0 / freq << "ms" << endl;
    deleteA();
