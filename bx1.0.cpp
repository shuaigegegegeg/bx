...
vector<int> columnVectorProduct(vector<vector<int>>& matrix, vector<int>& vector1) {
    int n = matrix.size();
...
int main() {
    ...
    //vector<int> result = columnVectorProduct(matrix, vector1);
    for (int i = 0; i < 20; i++) {
        columnVectorProduct(matrix, vector1);
    }

    QueryPerformanceCounter(&t2);
    cout << "TimeConsume: " << ((t2.QuadPart - t1.QuadPart) * 1.0 / tc.QuadPart) << endl;
    /*
    cout << "Resulting column vector products:" << endl;
